    ...
    override fun startAfterClientInit(startFuture: Future<Void>) {
        vertxClient.describeStream(getStreamName(), null, null, onSuccess<JsonObject> {
            ...
            for (shardId in shardIds) {
                vertx.getFromSharedMemoryAsync(
                    ShardIteratorMapName,
                    getShardIteratorKey(shardId),
                    onSuccess<String?> { shardIterator ->
                        ...
                        if (shardConsumerVerticleName == null) {
                            ...
                            vertx.deployVerticle(
                                ...
                                onSuccess<String> {
                                    ...
                                    latch.countDown()
                                        ...
                                        log.error(
                                            "Can't start shard consumer verticle",
                                            it
                                        )
                                    }
                            )
                            ...
                            log.error(
                                "Can't retrieve shard iterator from shared memory",
                                it
                            )
                        })
            }
