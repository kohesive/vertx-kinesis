...
class KinesisClientImpl(val vertx: Vertx, val config: JsonObject) : KinesisClient {
    ...
    override fun createStream(streamName: String, shardCount: Int?, resultHandler: Handler<AsyncResult<Any>>) {
        withClient { client ->
            client.createStreamAsync(CreateStreamRequest().withStreamName(streamName).withShardCount(shardCount), resultHandler.toKinesisHandler<CreateStreamRequest, CreateStreamResult>())
        }
    }
    ...
    fun <KinesisRequest : AmazonWebServiceRequest, KinesisResult: AmazonWebServiceResult<ResponseMetadata>> Handler<AsyncResult<Any>>.toKinesisHandler(): AsyncHandler<KinesisRequest, KinesisResult> {
        return withConverter { it }
    }

    fun <KinesisRequest : AmazonWebServiceRequest, KinesisResult: AmazonWebServiceResult<ResponseMetadata>, VertxResult> Handler<AsyncResult<VertxResult>>.withConverter(converter: (KinesisResult) -> VertxResult):
            KinesisToVertxHandlerAdapter<KinesisRequest, KinesisResult, VertxResult> {
        return KinesisToVertxHandlerAdapter(
            vertxHandler = this,
            kinesisResultToVertxMapper = converter
        )
    }

    class KinesisToVertxHandlerAdapter<KinesisRequest : AmazonWebServiceRequest, KinesisResult: AmazonWebServiceResult<ResponseMetadata>, VertxResult>(
